...
int perform(uint8_t* randbuf,size_t randbuf_len)
{
    ...
    unsigned int compressed_size = max_compressed_size;
        ...
        retv = BZ2_bzBuffToBuffCompress((char*)compressed_buf, &compressed_size, (char*)randbuf,
            (unsigned int)randbuf_len, 1, THIS_BZ_VERBOSITY, 0);
    if (retv != BZ_OK) {
    ...
    uint8_t* decompressed_buf = (uint8_t*)malloc(decompressed_size);
        ...
        retv = BZ2_bzBuffToBuffDecompress(
            ...
            &decompressed_size, 
            (char*)compressed_buf, compressed_size, 0, THIS_BZ_VERBOSITY);
    if (retv != BZ_OK) {
