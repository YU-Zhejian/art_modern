...
std::string ceu_check_get_compiler_info()
{
    ...
    oss << "\t"
        << "Intel Clang compatible version number: " << __INTEL_CLANG_COMPILER / 10000 << '.'
        << __INTEL_CLANG_COMPILER % 10000 / 100 << '.' << __INTEL_CLANG_COMPILER % 100 << std::endl;
#endif
#if defined(CEU_COMPILER_IS_AOCC)
    oss << "\t"
        << "AMD Optimizing C++ Compiler (AOCC) compatible version number: "
    ...
    oss << "\t"
        << "Intel Compiler Classic (ICC) compatible version number: " << std::to_string(__ICC)
...
#endif
    oss << "\t"
        << "MSVC compatible version number: " << msc_major_ver << '.' << msc_minor_ver << '.' << msc_internal_ver << '.'
        << msc_build_ver << ", with Visual Studio ver. " << CEU_VISUAL_STUDIO_VER;
#endif
    ...
    oss << "\t"
        << "PGI Compiler (PGIC) compatible version number: " << __PGIC__ << '.' << __PGIC_MINOR__ << '.'
    ...
    oss << "\t"
        << "Clang compatible version number: ";
    ...
    oss << "\t"
        << "__VERSION__ string: " << __VERSION__ << std::endl;
#endif
    return oss.str();
}
