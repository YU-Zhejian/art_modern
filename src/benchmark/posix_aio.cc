...
void bench(const std::string& path, const size_t block_size, const size_t num_blocks)
{
    ...
    std::cout << "AIO: " << std::setw(10) << to_si(block_size) << "B " << std::setw(10) << num_blocks << " "
              << describe(times) << " us; mean speed=" << to_si(block_size * num_blocks / mean(times) * 1000000)
              << "B/s" << std::endl;
    ...
    std::cout << "LIO: " << std::setw(10) << to_si(block_size) << "B " << std::setw(10) << num_blocks << " "
              << describe(times) << " us; mean speed=" << to_si(block_size * num_blocks / mean(times) * 1000000)
              << "B/s" << std::endl;
}
