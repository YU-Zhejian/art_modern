    ...
    void main_worker(){
        ...
        for (std::size_t thread_idx = 0; thread_idx < NUM_THREADS; ++thread_idx) {
            ...
            threads.emplace_back([thread_idx, &gss]() {
                ...
                BOOST_LOG_TRIVIAL(info) << "Rank " << mpi_rank() << " thread=" << thread_idx << " generated " << to_si(gss[thread_idx]->seeds_.size()) << " seeds.";
            });
        }
        for (auto& t : threads) {
            t.join();
        ...
        for (std::size_t thread_idx = 0; thread_idx < NUM_THREADS; ++thread_idx) {
            ...
            if (ser_size != position) {
                BOOST_LOG_TRIVIAL(fatal) << "Rank " << mpi_rank() << " thread=" << thread_idx << " serialized seeds size (" << ser_size << ") != position (" << to_si(position) << ")!";
                abort_mpi();
            }
            BOOST_LOG_TRIVIAL(info) << "Rank " << mpi_rank() << " thread=" << thread_idx << " serialized " << to_si(ser_size) << " bytes.";

            // Send via MPI
            BOOST_LOG_TRIVIAL(info) << "Rank " << mpi_rank() << " thread=" << thread_idx << " sending via MPI.";
            MPI_Send(buffer, static_cast<int>(ser_size), MPI_CHAR, MPI_MAIN_RANK, static_cast<int>(thread_idx), MPI_COMM_WORLD);
    ...
    void main_manager(){
        ...
        for (int source_rank = MPI_MAIN_RANK + 1; source_rank < mpi_size(); ++source_rank) {
            for (std::size_t thread_idx = 0; thread_idx < NUM_THREADS; ++thread_idx) {
                ...
                BOOST_LOG_TRIVIAL(info) << "Rank " << source_rank << " thread=" << thread_idx << " sized " << to_si(ser_size) << ".";
                ...
                if (ser_size != position) {
                    BOOST_LOG_TRIVIAL(fatal) << "Rank " << source_rank << " thread=" << thread_idx << " serialized seeds size (" << ser_size << ") != position (" << to_si(position) << ")!";
                    abort_mpi();
                }
