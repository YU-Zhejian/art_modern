...
namespace labw::art_modern {
    ...
    class GslDiscreteDistribution {
        size_t K;
        std::vector<size_t> A;
        std::vector<FloatType> F;
    public:
        GslDiscreteDistribution(): K(0) {};
        explicit GslDiscreteDistribution(const std::vector<FloatType>& prob_array) : K(prob_array.size())
        {
            ...
            F = std::vector<FloatType>(K);
            A = std::vector<size_t>(K);
