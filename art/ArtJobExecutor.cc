    ...
    for (size_t seq_id = 0; seq_id < job_.fasta_fetch->num_seqs(); ++seq_id) {
        ...
        if (job_.fasta_fetch->seq_len(seq_id) < art_params_.read_len) {
            BOOST_LOG_TRIVIAL(debug) << "Warning: the reference sequence " << contig_name << " (length "
                                     << job_.fasta_fetch->seq_len(seq_id) << "bps ) is skipped as it < the defined read length ("
...
//                                 << ": ArtContig created";
        ...
        if (art_params_.art_simulation_mode == SIMULATION_MODE::TEMPLATE) {
            ...
            num_pos_reads = static_cast<long>(
                coverage_positive * static_cast<double>(contig_size) / art_params_.read_len);
            num_neg_reads = static_cast<long>(
                coverage_negative * static_cast<double>(contig_size) / art_params_.read_len);
        }
