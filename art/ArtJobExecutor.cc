...
bool ArtJobExecutor::generate_pe(ArtContig& art_contig, const bool is_plus_strand)
{
    ...
    if (qual_1_str.size() != art_params_.read_len) {
        return false; // FIXME: No idea why this occurs.
    }
    if (qual_2_str.size() != art_params_.read_len) {
        return false; // FIXME: No idea why this occurs.
    }

    arp.read_1.generate_pairwise_aln();
    arp.read_2.generate_pairwise_aln();
    if (arp.read_1.aln_read.size() != arp.read_1.aln_ref.size()) {
        return false; // FIXME: No idea why this occurs.
    }
    if (arp.read_2.aln_read.size() != arp.read_2.aln_ref.size()) {
        return false; // FIXME: No idea why this occurs.
    }
...

{
    ...
    if (qual_str.size() != art_params_.read_len) {
        return false; // FIXME: No idea why this occurs.
    }
