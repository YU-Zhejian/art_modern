...
This example requires [gzip](https://www.gnu.org/software/gzip/), [pigz](https://zlib.net/pigz/), [SAMtools](https://github.com/samtools/samtools), and [XZ Utils](https://tukaani.org/xz/). Note that the `--i-type` parameter is required since now, th simulator cannot tell the file format through its file extension.
...
zcat opt/data/GCF_000005845.2_ASM584v2_genomic.fna.gz | \
    ...
    --o-fasta >(xz -9 -T5 -vv -cf - > opt/build_release/e_coli_wgs_se.fasta.xz) \
...
Message Passing Interface (MPI) is a standard for parallel computing on a high-performance cluster (HPC). It allows processes to communicate with each other through a network. If you're working on a cluster with [MPI](https://www.mpi-forum.org/) installed, you can use the MPI version of `art_modern` to speed up the simulation by executing `art_modern` on different nodes in parallel.
...
By default, the program would **NOT** support building profiles from NCBI short-read archive (SRA). Use `-DWITH_NCBI_NGS=ON` to enable it. Additional libraries (namely, `libncbi-ngs.so`s, with its development headers) are required.
