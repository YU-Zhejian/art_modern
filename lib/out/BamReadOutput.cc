...
namespace art_modern {
    ...
    void BamReadOutput::writeSE(const PairwiseAlignment& pwa)
    {
        ...
        int tid = CExceptionsProxy::assert_numeric(sam_hdr_name2tid(sam_header_, pwa.contig_name.c_str()),
        ...
        : sam_options_(std::move(sam_options))
    {
        ...
        sam_file_ = CExceptionsProxy::assert_not_null(sam_open(filename.c_str(), sam_options_.write_bam ? "wb" : "wh"),
        ...
        CExceptionsProxy::assert_numeric(sam_hdr_add_line(sam_header_, "HD", "VN", sam_options_.HD_VN.c_str(), "SO",
